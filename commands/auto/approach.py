...
from commands.driving.aim_to_direction import AimToDirectionConstants
from commands.driving.go_to_point import GoToPointConstants
...
class ApproachTag(commands2.Command):
    ...
    def initialize(self):
        ...
        log("Approach","Translation gain value {kpMultTran}, power={self.APPROACH_SHAPE.value}")
    ...
    def getVisionBasedSwerveDirection(self, now):
        ...
        if self.tReachedGlidePath != 0 and self.tReachedFinalApproach == 0 and robotX > 0:
            ...
            log("Approach", "Final approach starting from {self.xyReachedFinalApproach}")
    ...
    def hasReachedGlidePath(self, degreesLeftToRotate: float, distanceToGlidePath: float) -> bool:
        ...
        if self.tReachedGlidePath and not reachedNow:
            log("Approach","WARNING: not on glide path anymore (distance={distanceToGlidePath}, degrees={degreesLeftToRotate}")
        return reachedNow
...
class ApproachManually(commands2.Command):
    ...
    def initialize(self):
        ...
        log("ApproachTag", "Translation gain value {self.KPMULT_TRANSLATION.value}")
