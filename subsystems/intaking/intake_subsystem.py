...
class IntakeRollers:
    ...
    def update(self):
        """
        Called every loop. Publishes roller telemetry.
        Must be called from IntakeSubsystem.periodic().
        """
        Logger.recordOutput("Intake/Rollers/ActualSpeedRPS", self.intakeMotor.get_velocity().value)
        Logger.recordOutput("Intake/Rollers/TargetSpeedRPS", self.velocity)
        Logger.recordOutput("Intake/Rollers/SupplyCurrent", self.intakeMotor.get_supply_current().value)
        Logger.recordOutput("Intake/Rollers/StatorCurrent", self.intakeMotor.get_stator_current().value)
    ...
    def intake_reverse(self):
        self.velocity = -IntakeConstants.kIntakeSpeed
        self.intakeMotor.set_control(self.intakeRequest.with_velocity(self.velocity))
        Logger.recordOutput("Intake/Rollers/TargetSpeedRPS", self.velocity)
    ...
    def stop(self):
        ...
        Logger.recordOutput("Intake/Rollers/TargetSpeedRPS", 0)
...
class IntakePivot:
    ...
    def update(self):
        """
        Called every loop. Handles homing logic, limit switch syncing, and telemetry.
        Must be called from IntakeSubsystem.periodic().
        """
        Logger.recordOutput("Intake/Pivot/Homed", self._homed)
        Logger.recordOutput("Intake/Pivot/Deployed", self._isDeployed)
        Logger.recordOutput("Intake/Pivot/ForwardLimit", self.forward_limit_pressed())
        Logger.recordOutput("Intake/Pivot/ReverseLimit", self.reverse_limit_pressed())
        Logger.recordOutput("Intake/Pivot/Position", self.deployEncoder.getPosition())
        Logger.recordOutput("Intake/Pivot/Setpoint", self._setpoint)
        Logger.recordOutput("Intake/Pivot/Current", self.deployMotor.getOutputCurrent())
